...
function mapConfigImport(){
    ...
    reader.onload = function () {
        ...
        //  Save current frame
        frames[frame - 1] = cloneData();
        // var generateMapJsonjs = document.createElement('script');
        // generateMapJsonjs.id = 'generateMapJsonjs';
        // generateMapJsonjs.src = "generateMapJson.js";
        // document.body.appendChild(generateMapJsonjs);

        // var generateAndDownload = function(){
        tilemap_data = {};
        // console.log(generate_tilemap_data);
        generate_tilemap_data(frames, tilemap_data ,function(){
            var date = new Date();
            var filename = date.toLocaleDateString().split('/').slice(2,3).concat(
                date.toLocaleDateString().split('/').splice(0,2)).join('-') + " " + (
                date.toLocaleTimeString().split(' ')[1]=='AM' ?
        ...
        // if(document.getElementById('generateMapJsonjs')){
        // }else{
        //     generateMapJsonjs.onload = function(){
        //         generateAndDownload();
        //     };
        // }
